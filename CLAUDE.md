...
- ✅ 新增股票：先 `python build_data.py {code}`，再在 HTML 添加 `<script>` 标签，**同时更新 `.gitignore` 白名单**
- ✅ 新增策略：在 `strategy.js`（JS 版，在线回退用）+ `build_data.py`（Python 版，预计算用）同步实现
- ✅ CSS 中所有 URL 引用必须是本地相对路径
- ✅ `onStrategySelect` 和 `onPeriodSelect` **必须同时处理 localData 和 online 两种数据源**（见 Debug #14）
...
图表左侧有纵向排列的 BOLL(55,2) 和 MA(5,20) 按钮，互斥开关：
- **BOLL**：上轨蓝、中轨橘、下轨紫，实线 1.2px
- **MA**：MA5 黄、MA20 粉，实线 1.2px
- **互斥逻辑**：`chart.js` 中 `toggleBollinger()` 和 `toggleMA()` 互相置反；`app.js` 中点击时同步更新 DOM active 状态
- **布局**：按钮放在 `chart-overlay-btns` 容器中（`chart-body` flex 左侧），不覆盖图表

## SVG 图标规范

所有界面图标使用内联 SVG（非 emoji），统一 `.label-icon` 类：
- 尺寸：22px × 22px，透明度 0.8
- 颜色继承父元素 `currentColor`
- 使用 `display: flex; align-items: center` 与文字对齐
- 占位图标：K线柱 SVG（90×56px），绿涨红跌

## 买卖点标记

- **买入**：棕色圆点 `#b0723a`，`symbol: 'circle'`，size 10
- **卖出**：深蓝三角 `#1d4ed8`，`symbol: 'triangle'`，`symbolRotate: 180`，size 14
- **执行价格**：次日开盘价（`idx+1.open`），tooltip 中标注 `(次日开 xxx.xx)`

## 名称搜索

`app.js` 中 `runBacktest()` 会检测输入是否包含中文（`/[一-鿿]/`），自动在 `STOCK_LIST` 中匹配名称→代码转换。搜索同时支持代码和名称。
...
### 9. 名称搜索需要先匹配 STOCK_LIST

用户输入中文名称时 `detectMarket()` 无法识别，会将其当作深市代码直接查询，导致 `findLocalData()` 失败并回退到在线 API（`failed fetch`）。**必须先检测中文/特殊字符 → 遍历 STOCK_LIST 匹配 → 解析为代码**。

### 10. git revert 会丢失未提交的新功能

`git checkout --` 回退文件时会丢弃该文件中的所有后续修改（如 BOLL 按钮、MA 按钮、CSS 样式）。**回退前确认需要保留的功能，回退后逐一检查和修复**。

### 11. 下拉框需要高于后续区块

`autocomplete-dropdown` 需要 `z-index: 200`，`input-section` 需要 `z-index: 10`，后续区块需要 `z-index: 1`，否则下拉框会被回测周期等卡片遮挡。

### 12. 预下载的股票必须同步到 STOCK_LIST 和 index.html 和 .gitignore

新增预下载股票时需**四步**（缺一不可）：
1. `python build_data.py {code}` — 生成数据文件
2. `constants.js` 中 `STOCK_LIST` 添加条目 — 支持搜索
3. `index.html` 中添加 `<script src="data/{code}.js">` — 加载数据
4. **`.gitignore` 中添加 `!data/{code}.js`** — 否则不会部署到 Pages

> 漏掉第 4 步的后果：本地正常但 Pages 上无预计算数据，回退到在线 API + JS 计算，所有策略显示相同收益率（因为 Debug #14 的 bug）。

### 13. .gitignore 白名单必须与预下载股票同步

`.gitignore` 规则 `data/*.js` 会排除所有数据文件，只有 `!data/{code}.js` 白名单的文件才会提交到 git 并部署。每次 `build_data.py` 新股票后必须同步更新白名单。

### 14. onStrategySelect/onPeriodSelect 必须双分支处理

当前数据来源有两种：预计算（`state.localData` 非空）和在线 API（`state.localData` 为 null）。切换策略/周期时**必须两个分支都处理**：

```javascript
function onStrategySelect(sid) {
    if (state.strategy === sid) return;
    state.strategy = sid;
    if (state.localData) { reload(); return; }       // 预计算分支
    if (state.klines) {                                // 在线数据分支
...
> 漏掉在线数据分支的后果：无预计算数据时切换策略，图表和指标完全不更新，所有策略显示相同收益率。

### 15. 手机端布局：chart-panel 负 margin + BOLL/MA 绝对定位

手机端图表要占满屏幕宽度：
- `.chart-panel.card` 设置 `margin-left/right: -6px; width: calc(100% + 12px); padding: 0; border: none`
- BOLL/MA 按钮用 `position: absolute; top: 6px; right: 6px` 浮在图表上
- 周期按钮用 `display: grid; grid-template-columns: repeat(3, 1fr)` 等宽排列
- 搜索框和查询按钮用 `flex-direction: row; flex-wrap: nowrap` 保持同行
